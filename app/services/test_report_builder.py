...
class TestReportBuilder:
    ...
    def build_html_report(test_run, repository=None, run_number: int = 1) -> str:
        ...
        if repository:
            ...
            repo_url = escape(
                str(
                    getattr(repository, "repository_url", "")
                    or getattr(repository, "url", "")
                    or ""
                )
            )
            ...
            for i, tc in enumerate(test_cases, 1):
                ...
                <tr style="border-bottom:1px solid #1e293b;">
                    ...
                    <td style="padding:12px 16px;text-align:center;">
                        <span style="background:{pill_bg};color:#fff;padding:4px 12px;border-radius:4px;font-size:12px;font-weight:bold;">{tc_status}</span>
                    </td>
                    <td style="padding:12px 16px;color:#94a3b8;text-align:center;font-size:13px;">{tc_time}</td>
                    <td style="padding:12px 16px;color:#94a3b8;text-align:center;font-size:13px;">{tc_message[:80]}</td>
                </tr>""")

            test_cases_html = "\n".join(rows)

        # Build the summary counter cards
        ...
        # Build test detail section
        test_detail_section = ""
        if junit_data and junit_data.get("test_cases"):
            test_detail_section = f"""
            <div style="margin-top:32px;">
                <h2 style="font-size:18px;color:#a5f3fc;margin-bottom:16px;">
                    ✏️ DETALLE DE TESTS ({total} PRUEBAS)
                </h2>
                <table style="width:100%;border-collapse:collapse;background:#151c2c;border-radius:8px;overflow:hidden;">
                    <thead>
                        <tr style="background:#1e293b;">
                            ...
                            <th style="padding:12px 16px;color:#e2e8f0;text-align:center;font-size:13px;">Mensaje</th>
                        </tr>
                    </thead>
                    <tbody>
                        {test_cases_html}
        ...
        if stderr:
            ...
            stderr_section = f"""
            <div style="margin-top:32px;">
                <h2 style="font-size:18px;color:#fca5a5;margin-bottom:16px;">
                    📥 STDERR
                </h2>
        ...
        header_title = f"[RUN #{run_number}] {repo_name} · CI/CD PIPELINE" if repo_name else f"[RUN #{run_number}] CI/CD PIPELINE"
